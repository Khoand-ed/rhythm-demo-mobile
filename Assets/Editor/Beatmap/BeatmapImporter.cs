...
public class BeatmapImporter
{
    ...
    [MenuItem("Beatmap/Import All Beatmaps", false, 20)]
    public static void ImportAll()
    {
        ...
        float noteWidth = ChartSpacing.MeasureNoteWidth();
        ...
        foreach (string songFolder in AssetDatabase.GetSubFolders(BeatmapsRoot))
        {
            ...
            // Same fallback as ImportFile, or a folder whose only clip is not
            // called main would have its charts' audio wiped on every import.
            if (main == null) main = LoadOnlyClip(songFolder);

            if (main == null) Debug.LogWarning($"{songFolder}: no main audio found (expected main.mp3); charts keep whatever clip they already had.");
        ...
        if (imported.Count > 0) UpdateLibrary(imported, replaceAll: true);
    ...
    // Imports one beatmap json into the SongChart beside it and adds that
    // chart to the library without touching the others. Used by the Beatmap
    // Studio's Generate + Import, so a generated chart is playable in one step.
    public static SongChart ImportFile(string jsonPath)
    {
        jsonPath = jsonPath.Replace('\\', '/');
        string songFolder = System.IO.Path.GetDirectoryName(jsonPath).Replace('\\', '/');

        AudioClip main = LoadFirst<AudioClip>(songFolder, "main");
        AudioClip demo = LoadFirst<AudioClip>(songFolder, "demo");
        Sprite cover = LoadFirst<Sprite>(songFolder, "cover");

        // A folder the generator was pointed at may only hold the one clip the
        // chart was made from, under any name - better that than no audio.
        if (main == null) main = LoadOnlyClip(songFolder);
        if (main == null) Debug.LogWarning($"{songFolder}: no main audio found (expected main.mp3); charts keep whatever clip they already had.");
        ...
        SongChart chart = ImportOne(jsonPath, main, demo, cover, ChartSpacing.MeasureNoteWidth(), BuildLaneMap());
        if (chart == null) return null;

        UpdateLibrary(new List<SongChart> { chart }, replaceAll: false);
        AssetDatabase.SaveAssets();

        return chart;
    }

    private static AudioClip LoadOnlyClip(string folder)
    {
        ...
        foreach (string guid in AssetDatabase.FindAssets("t:AudioClip", new[] { folder }))
        {
            ...
            if (System.IO.Path.GetFileNameWithoutExtension(path).ToLowerInvariant() == "demo") continue;

            // More than one candidate: guessing would be worse than warning.
            if (found != null) return null;
            found = AssetDatabase.LoadAssetAtPath<AudioClip>(path);
        }
                                       ...
                                       float noteWidth, Dictionary<string, int> laneIndexByName)
    {
        ...
        float minGap;
        float speed = ChartSpacing.ResolveSpeed(notes, baseSpeed, noteWidth, out minGap);
        ...
        chart.bpm = json.bpm;
        chart.beatPhase = json.beatPhase;
        chart.difficulty = json.difficulty;
        chart.offset = json.offset;
        chart.noteSpeed = speed;
        chart.notes = notes;
        // A chart whose audio lives outside its folder (Beatmap Studio can
        // generate from any clip) keeps the clip it already has, rather than
        // being silenced by a re-import.
        if (main != null) chart.clip = main;
        chart.demoClip = demo;
        chart.cover = cover;
    ...
    // A full import rebuilds the list, so deleted songs drop out; a single
    // import only adds or refreshes its own chart.
    private static void UpdateLibrary(List<SongChart> charts, bool replaceAll)
    {
        ...
        if (replaceAll || library.charts == null)
        {
            library.charts = new List<SongChart>(charts);
        }
        else
        {
            library.charts.RemoveAll(existing => existing == null || charts.Contains(existing));
            library.charts.AddRange(charts);
        }
