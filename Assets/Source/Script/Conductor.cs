...
public class Conductor : MonoBehaviour
{
    ...
    // Jumps ahead mid-song (the intro Skip). Unlike Seek this restarts the
    // audio on a scheduled dsp boundary, so it also works during the silent
    // run-up - when PlayScheduled has not started the clip yet and setting
    // AudioSource.time alone would be overridden by the pending start.
    public void SkipTo(float songTime)
    {
        if (!IsPlaying || IsPaused || theMusic.clip == null) return;

        // Audio position and song time differ by the sync offsets, the same
        // relationship StartSong sets up.
        float audioTime = Mathf.Clamp(songTime + TotalOffset, 0f, theMusic.clip.length - 0.01f);
        double startAt = AudioSettings.dspTime + 0.05;

        theMusic.Stop();
        theMusic.time = audioTime;
        theMusic.PlayScheduled(startAt);

        songStartDsp = startAt - audioTime;
    }
