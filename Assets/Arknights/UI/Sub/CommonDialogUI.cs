...
namespace UI.Sub {
    public class CommonDialogUI : UIBase {
        ...
        public static CommonDialogUI Message(GroundType type,string message) {
            CommonDialogUI ui = Open(message);
        ...
        /// <summary>
        /// 对话框预制体缺失时不要直接空引用, 把原本要显示的内容打到控制台
        /// When the dialog prefab is missing, UIManager.Show returns null. Report the message
        /// that would have been shown instead of throwing, so the caller can keep going.
        /// </summary>
        private static CommonDialogUI Open(string message) {
            CommonDialogUI ui = UIManager.Inst().Show("CommonDialogUI") as CommonDialogUI;
            if (ui == null) {
                Debug.LogError($"[CommonDialogUI] 无法显示对话框 / Cannot show dialog - the " +
                               $"'CommonDialogUI' prefab is missing. Message was: {message}");
            }
            return ui;
        }
